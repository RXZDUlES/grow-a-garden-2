...
local CONFIG_PATH = "RXZ_HUB/steal_config.json"
local Radii = {Semi = 9}
...
local function refreshModeButtons()
    for mode, btn in pairs(ALL_MODE_BTNS) do
        ...
        btn.BackgroundColor3 = active and Color3.fromRGB(210, 210, 210) or Color3.fromRGB(60, 60, 60)
        btn.TextColor3       = active and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(230, 230, 230)
    end
end
...
-- ===== RXZ HUB STEAL BAR (1..100, shows only while stealing) =====
pcall(function()
    local old = CoreGui:FindFirstChild("RXZ_StealBar"); if old then old:Destroy() end
end)
local sbGui = Instance.new("ScreenGui")
sbGui.Name = "RXZ_StealBar"; sbGui.ResetOnSpawn = false
sbGui.IgnoreGuiInset = true; sbGui.DisplayOrder = 30
pcall(function() if syn and syn.protect_gui then syn.protect_gui(sbGui) end end)
if not pcall(function() sbGui.Parent = CoreGui end) then sbGui.Parent = LP:WaitForChild("PlayerGui") end

local sbHolder = Instance.new("Frame", sbGui)
sbHolder.Name = "Holder"
sbHolder.AnchorPoint = Vector2.new(0.5, 0.5)
sbHolder.Size = UDim2.new(0, 240, 0, 10)
sbHolder.Position = UDim2.new(0.5, 0, 0.5, 70)
sbHolder.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
sbHolder.BorderSizePixel = 0
sbHolder.Visible = false
sbHolder.ZIndex = 40
Instance.new("UICorner", sbHolder).CornerRadius = UDim.new(1, 0)
local sbStroke = Instance.new("UIStroke", sbHolder)
sbStroke.Color = Color3.fromRGB(255, 255, 255); sbStroke.Thickness = 1; sbStroke.Transparency = 0.4

local sbFill = Instance.new("Frame", sbHolder)
sbFill.Size = UDim2.new(0, 0, 1, 0)
sbFill.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
...
ProgressBarFill = sbFill
ProgressPercentLabel = sbPct

task.spawn(function()
    while sbGui.Parent do
        task.wait(0.05)
        local active = false
        pcall(function() active = (A and A.state and A.state.active) == true end)
        sbHolder.Visible = active
        if not active then
            sbFill.Size = UDim2.new(0, 0, 1, 0)
        end
    end
...
startAll()
task.spawn(function()
    while sbGui.Parent do task.wait(30); saveConfig() end
end)
end) end)
repeat task.wait() until game:IsLoaded()
...
local TOGGLE_ON_COLOR = Color3.fromRGB(255, 255, 255)
...
-- ============================================
-- GAME PROTECTION / ANTI KICK (safe mode)
-- ============================================
antiKickEnabled  = true
brainrotDetected = false

local function _hasBrainrotCarry(char)
    ...
    for _, t in ipairs(char:GetChildren()) do
        if t:IsA("Tool") then
            ...
            local ignored = nm:find("bat") or nm:find("slap") or nm:find("medusa") or nm:find("head") or nm:find("stone")
            if not ignored and (nm:find("brainrot") or nm:find("animal") or nm:find("carry")
                or nm:find("grab") or nm:find("steal") or nm:find("hold")) then
                return true
            end
        end
    end
...
function safeModeHoldingBrainrot()
    local ok, res = pcall(_hasBrainrotCarry, LP.Character)
...
task.spawn(function()
    while true do
        task.wait(0.25)
        brainrotDetected = safeModeHoldingBrainrot()
    end
end)

-- block client-side kicks coming from the game anticheat
pcall(function()
...
pcall(function()
    if hookfunction and newcclosure and LP and LP.Kick then
        hookfunction(LP.Kick, newcclosure(function() return nil end))
    end
end)
pcall(function()
    if hookfunction and newcclosure and game.Shutdown then
        hookfunction(game.Shutdown, newcclosure(function() return nil end))
    end
end)
...
local buttonVisibility = {
    ...
    instaReset=true, taunt=true,
}
-- Auto drop + aimbot when TP BAT / BAT V2 enabled while carrying (always on)
local function refreshAPExtBtns()
    local COL_ON  = Color3.fromRGB(255, 255, 255)
    local COL_OFF = Color3.fromRGB(65, 65, 65)
...
local BTN_OFF = Color3.fromRGB(140, 140, 140)
...
RunService.RenderStepped:Connect(function()
    ...
    if tpLockEnabled or autoLeftEnabled or autoRightEnabled or boxAutoPlayEnabled or batV2Enabled or autoBatEnabled then
...
-- ========== AUTO LEFT / AUTO RIGHT ENGINE ==========
local leftWaypoints = {
    Vector3.new(-476.85, -6.59, 94.91),   -- going 1
    Vector3.new(-485.55, -4.53, 100.61),  -- going 2
...
local rightWaypoints = {
    Vector3.new(-475.77, -6.57, 26.76),   -- going 1
    Vector3.new(-485.85, -4.48, 20.13),   -- going 2
...
function setAutoDirection(dir)
    local wantOff = boxAutoPlayEnabled and BOX_AP_Direction == dir
    if wantOff then
        boxAutoPlayEnabled = false
        autoLeftEnabled, autoRightEnabled = false, false
        stopBoxAutoPlay()
        ...
        if boxAutoPlayEnabled then stopBoxAutoPlay() end
        BOX_AP_Direction = dir
        if tpLockEnabled then toggleTPLock() end
        if batV2Enabled  then toggleBatV2()  end
        boxAutoPlayEnabled = true
        autoLeftEnabled  = (dir == "left")
        autoRightEnabled = (dir == "right")
        startBoxAutoPlay()
    end
    if mobBtnRefs.autoLeft  then mobBtnRefs.autoLeft(boxAutoPlayEnabled and BOX_AP_Direction == "left")  end
    if mobBtnRefs.autoRight then mobBtnRefs.autoRight(boxAutoPlayEnabled and BOX_AP_Direction == "right") end
    refreshAPExtBtns()
    pcall(saveConfig)
    return boxAutoPlayEnabled
end

function startBoxAutoPlay()
    if boxConn then boxConn:Disconnect() end
    boxWaypointIndex = 1; boxCurrentPhase = 1
    _boxAPCarrySwitched = false
    local function _wps() return (BOX_AP_Direction == "left") and leftWaypoints or rightWaypoints end
    ensureBoxProxy()
    refreshAPExtBtns()
    boxConn = RunService.Stepped:Connect(function()
        ...
        if dist < 2.5 then
            -- RXZ: stop right at the brainrot (waypoint 2), do NOT walk back
            if boxWaypointIndex == 2 then
                if boxConn then boxConn:Disconnect(); boxConn = nil end
                _boxStopMoving()
...
function saveConfig()
    ...
    pcall(function() writefile("RXZ_HUB_PC.json", HS:JSONEncode(cfg)) end)
end
...
local function createHeadLabels(char)
    ...
    overheadGui.Name = "RXZ_OverheadInfo"
    ...
    discordLbl.Text = "discord.gg/rxz"
...
local function createPlayerSpeedLabel(plr)
    ...
    speedGui.Name = "RXZSpeedLabel_Other"
...
LP.CharacterAdded:Connect(function(char)
    ...
    startAntiDie()
    -- Auto Left/Right reconnect on respawn
    if boxAutoPlayEnabled then
        local _dir = BOX_AP_Direction
        stopBoxAutoPlay()
        task.wait(0.8)
        setAutoDirection(_dir)
    end
end)
...
function buildMobileButtons()
    ...
    mobGui.Name = "RXZMobileButtons"; mobGui.ResetOnSpawn = false
    ...
    -- RXZ stack style (black / white)
    local QS = math.clamp(math.floor(mobileButtonsSize * 0.72), 44, 130)
    local QG = 8
    local QR = 12
    local Q_OFF       = Color3.fromRGB(8, 8, 8)
    local Q_ON        = Color3.fromRGB(235, 235, 235)
    local Q_BORDER    = Color3.fromRGB(60, 60, 60)
    local Q_BORDER_ON = Color3.fromRGB(255, 255, 255)
    local Q_TEXT      = Color3.fromRGB(190, 190, 190)
    local Q_TEXT_ON   = Color3.fromRGB(10, 10, 10)

    local QCOLS, QROWS = 2, 5
    local QW = QS * QCOLS + QG * (QCOLS - 1)
    local QH = QS * QROWS + QG * (QROWS - 1)

    local mbGroup = Instance.new("Frame", mobGui)
    mbGroup.Name = "MobileButtons"
    mbGroup.Size = UDim2.new(1, 0, 1, 0)
    mbGroup.BackgroundTransparency = 1
    mbGroup.BorderSizePixel = 0
    mbGroup.ZIndex = 100

    local function makeMobileBtn(key, label, col, rowN, isToggle, onAction)
        local frame = Instance.new("Frame", mbGroup)
        frame.Name = "MobBtn_" .. key
        frame:SetAttribute("BtnKey", key)
        frame.Size = UDim2.new(0, QS, 0, QS)
        frame.Position = UDim2.new(1, -QW - 20 + col * (QS + QG), 0.5, -QH/2 + rowN * (QS + QG))
        local sp = savedPositions[key]
        if type(sp) == "table" and sp.xo then
            local vp = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1280, 720)
            local xo, yo = sp.xo, sp.yo or 0
            -- ignore saved spots that would put the button off screen
            if xo > -QS and yo > -QS and xo < vp.X - 10 and yo < vp.Y - 10 then
        ...
        stroke.Color = Q_BORDER; stroke.Thickness = 0
        ...
        btn.TextSize = math.max(10, math.floor(QS * 0.19))
        ...
        local function setOn(s)
            isOn = s
            frame:SetAttribute("BtnIsOn", s)
            TS:Create(frame, TweenInfo.new(0.18, Enum.EasingStyle.Quad), {BackgroundColor3 = s and Q_ON or Q_OFF}):Play()
            TS:Create(stroke, TweenInfo.new(0.18), {Color = s and Q_BORDER_ON or Q_BORDER}):Play()
            btn.TextColor3 = s and Q_TEXT_ON or Q_TEXT
        end
        mobBtnRefs[key] = setOn

        local _dn, _sp, _fp, _wd = false, nil, nil, false
        ...
        btn.InputChanged:Connect(function(i)
            if not _dn or uiLocked or not perButtonDragEnabled then return end
            if i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch then
                local dx = i.Position.X - _sp.X
                local dy = i.Position.Y - _sp.Y
                if math.abs(dx) > 6 or math.abs(dy) > 6 then _wd = true end
                if _wd then
        ...
        btn.MouseButton1Click:Connect(function()
            if _wd then _wd = false; return end
            if not isToggle then
                TS:Create(frame, TweenInfo.new(0.08), {BackgroundColor3 = Q_ON}):Play()
                btn.TextColor3 = Q_TEXT_ON
                task.delay(0.2, function()
                    TS:Create(frame, TweenInfo.new(0.15), {BackgroundColor3 = Q_OFF}):Play()
                    btn.TextColor3 = Q_TEXT
                end)
            end
            if onAction then onAction(setOn) end
        end)
        return setOn
    end

    local defs = {
        {key="drop",        label="DROP\nBR",      col=0, row=0, toggle=false},
        {key="batV2",       label="BAT\nMODE",     col=0, row=1, toggle=true},
        {key="carrySpeed",  label="CARRY\nSPD",    col=0, row=2, toggle=true},
        {key="laggerCarry", label="LAGGER\nCARRY", col=0, row=3, toggle=true},
        {key="tpLock",      label="TP\nBAT",       col=0, row=4, toggle=true},
        {key="autoLeft",    label="AUTO\nLEFT",    col=1, row=0, toggle=true},
        {key="autoRight",   label="AUTO\nRIGHT",   col=1, row=1, toggle=true},
        {key="tpDown",      label="TP\nDOWN",      col=1, row=2, toggle=false},
        {key="lagger",      label="LAGGER\nMODE",  col=1, row=3, toggle=true},
    }
    ...
    local function setAutoPlay(dir)
        setAutoDirection(dir)
        syncAuto()
    end

    for _, d in ipairs(defs) do
        if buttonVisibility[d.key] ~= false then
            makeMobileBtn(d.key, d.label, d.col, d.row, d.toggle, function(setOn)
                if d.key == "autoLeft" then setAutoPlay("left"); return end
                if d.key == "autoRight" then setAutoPlay("right"); return end
                if d.key == "tpLock" then
                    if batV2Enabled then toggleBatV2() end
...
local function loadConfig()
    if not (isfile and isfile("RXZ_HUB_PC.json")) then return end
    local ok, cfg = pcall(function() return HS:JSONDecode(readfile("RXZ_HUB_PC.json")) end)
...
if _G.RXZ_HUB_LOADED then return end
_G.RXZ_HUB_LOADED = true

local function buildGui()
    local WHITE    = Color3.fromRGB(255, 255, 255)
    local BLACK    = Color3.fromRGB(0,   0,   0)
    local DARK     = Color3.fromRGB(14, 14, 14)
    local DARKER   = Color3.fromRGB(18, 18, 18)
    local MID      = Color3.fromRGB(40,  40,  40)
    local DIM_LINE = Color3.fromRGB(160, 160, 160)
    local CHECK_OFF= Color3.fromRGB(110, 110, 110)
    local BTN_ACT  = Color3.fromRGB(200, 200, 200)
    local INP_BG   = Color3.fromRGB(26, 26, 26)

    for _, name in ipairs({"RXZ HUB", "RXZMobileButtons", "RXZ_StealBar", "RXZRagdollTimer"}) do
    ...
    gui.Name = "RXZ HUB"; gui.ResetOnSpawn = false
    ...
    mainFrame.Position = UDim2.new(0.02, 0, 0.5, -310)
    ...
    Title.BackgroundTransparency = 1; Title.Text = "RXZ HUB"
    ...
    HeaderLine.BackgroundColor3 = Color3.fromRGB(220, 220, 220); HeaderLine.BackgroundTransparency = 0.5
    ...
    miniStroke.Color = Color3.fromRGB(255, 255, 255); miniStroke.Thickness = 2.5

    local miniShadow = Instance.new("UIStroke", miniToggleBtn)
    miniShadow.Color = Color3.fromRGB(150, 150, 150); miniShadow.Thickness = 4
    ...
    UIS.InputChanged:Connect(function(input)
        if miniDrag and not uiLocked and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
    ...
    SidebarLine.BackgroundColor3 = Color3.fromRGB(220, 220, 220); SidebarLine.BorderSizePixel = 0; SidebarLine.ZIndex = 3
    ...
    ContentContainer.Size = UDim2.new(1,-24,1,-84); ContentContainer.Position = UDim2.new(0,12,0,74)
    ...
    local function mkLabel(row, txt)
        ...
        l.Size = UDim2.new(1,-120,1,0); l.Position = UDim2.new(0,0,0,0)
    ...
    local function mkToggle(parent, txt, cb, initVal)
        ...
        chk.Size = UDim2.new(0,44,0,22); chk.Position = UDim2.new(1,-50,0.5,-11)
        chk.BackgroundColor3 = initVal and TOGGLE_ON_COLOR or CHECK_OFF
        chk.BorderSizePixel = 0; chk.Text = ""; chk.AutoButtonColor = false; chk.ZIndex = 5
        Instance.new("UICorner", chk).CornerRadius = UDim.new(1,0)
        local knob = Instance.new("Frame", chk)
        knob.Size = UDim2.new(0,18,0,18); knob.Position = UDim2.new(0,2,0.5,-9)
        knob.BackgroundColor3 = WHITE; knob.BorderSizePixel = 0; knob.ZIndex = 6
        ...
        local function sv(s)
            ...
            TS:Create(knob, TweenInfo.new(0.15), {Position = s and UDim2.new(1,-20,0.5,-9) or UDim2.new(0,2,0.5,-9)}):Play()
        end
        sv(state)
    ...
    local function mkToggleKB(parent, txt, kbEntry, onToggle, onKB)
        ...
        local chk = Instance.new("TextButton", row)
        chk.Size = UDim2.new(0,44,0,22); chk.Position = UDim2.new(1,-50,0.5,-11)
        chk.BackgroundColor3 = CHECK_OFF; chk.BorderSizePixel = 0; chk.Text = ""
        chk.AutoButtonColor = false; chk.ZIndex = 5
        Instance.new("UICorner", chk).CornerRadius = UDim.new(1,0)
        local knob = Instance.new("Frame", chk)
        knob.Size = UDim2.new(0,18,0,18); knob.Position = UDim2.new(0,2,0.5,-9)
        knob.BackgroundColor3 = WHITE; knob.BorderSizePixel = 0; knob.ZIndex = 6
    ...
    do
        local _allPage = TabRefs.contents["ALL"]
        for _, n in ipairs({"Speed","Combat","Mechns","Visual","Extra","Songs","Mobile"}) do
            TabRefs.contents[n] = _allPage
        end
        Sidebar.Visible = false
        ContentContainer.Size = UDim2.new(1, -24, 1, -84)
    ...
    mkActionRow(sfCombat, "Taunt",         function()
        local message = "RXZ HUB is better ur trash lol"
    ...
    do
        ...
        local function mkJumpModeBtn(lbl, xOff, mode)
            ...
            b.TextColor3 = (infJumpMode==mode) and BLACK or Color3.fromRGB(200, 200, 200)
            ...
            b.MouseButton1Click:Connect(function()
                ...
                for _, ob in ipairs(jBtns) do
                    if ob ~= b then
                        ob.BackgroundColor3 = BTN_ACT; ob.TextColor3 = Color3.fromRGB(200, 200, 200)
                    end
                end
                stopHoldInfJump()
    ...
    mkSect(sfMechns, "Auto Left / Right")

    do
        local dirRow = mkRow(sfMechns)
        mkLabel(dirRow, "Auto Run")
        local function mkDirBtn(label, xOffset)
            ...
            local bs = Instance.new("UIStroke", b); bs.Thickness = 1; bs.Color = Color3.fromRGB(255,255,255); bs.Transparency = 0.6
            return b
        end
        local btnRight = mkDirBtn("RIGHT", -8)
        local btnLeft  = mkDirBtn("LEFT",  -76)
        local function refreshDirBtns()
            local ON  = Color3.fromRGB(255, 255, 255)
            local OFF = Color3.fromRGB(55, 55, 55)
            local lOn = boxAutoPlayEnabled and BOX_AP_Direction == "left"
            local rOn = boxAutoPlayEnabled and BOX_AP_Direction == "right"
            btnLeft.BackgroundColor3  = lOn and ON or OFF
            btnRight.BackgroundColor3 = rOn and ON or OFF
            btnLeft.TextColor3  = lOn and Color3.fromRGB(0,0,0) or Color3.fromRGB(255,255,255)
            btnRight.TextColor3 = rOn and Color3.fromRGB(0,0,0) or Color3.fromRGB(255,255,255)
        end
        apExtSetLeft  = function() refreshDirBtns() end
        apExtSetRight = function() refreshDirBtns() end
        refreshDirBtns()
        btnLeft.MouseButton1Click:Connect(function()
            setAutoDirection("left"); refreshDirBtns()
        end)
        btnRight.MouseButton1Click:Connect(function()
            setAutoDirection("right"); refreshDirBtns()
        end)
    end
    ...
    local function playSoundFile(fileName, url, assetId)
        stopCurrentSong()
        if assetId and tostring(assetId) ~= "" then
    ...
    local INTRO_SONGS = {
        -- id = optional rbxassetid (plays instantly without downloading)
        {name = "Lady In Red", file = "lady_red_intro.mp3", url = "https://litter.catbox.moe/z1aao3.mp3", id = nil},
        {name = "Nuts",        file = "theeditr7.mp3",      url = "https://litter.catbox.moe/s42v4v.mp3", id = nil},
    }
    ...
    local function makeCustomSongRow(songName, fileName, songUrl, songId)
        ...
        siBtn.BackgroundColor3 = Color3.fromRGB(140, 140, 140)
        ...
        playBtn.Size = UDim2.new(0,42,0,22)
        playBtn.Position = UDim2.new(1,-46,0.5,-11)
        ...
        pStroke.Color = Color3.fromRGB(255,255,255); pStroke.Thickness = 1
        siBtn.MouseButton1Click:Connect(function()
            ...
            refreshIntroLabel()
            saveConfig()
            TS:Create(siBtn, TweenInfo.new(0.08), {BackgroundColor3 = Color3.fromRGB(200, 200, 200)}):Play()
            task.delay(0.3, function()
                TS:Create(siBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(140, 140, 140)}):Play()
            end)
        end)
        playBtn.MouseButton1Click:Connect(function()
            playSoundFile(fileName, songUrl, songId)
        end)
    end

    for _, opt in ipairs(INTRO_SONGS) do
        makeCustomSongRow(opt.name, opt.file, opt.url, opt.id)
    end
    ...
    do
        ...
        local mobLockSet = mkToggle(sfMobile, "Lock All (No Moving)", function(on)
            uiLocked = on
    ...
    mainFrame.Visible = false
    miniToggleBtn.Visible = false
    print("RXZ HUB GUI Fully Loaded")
end

local function _runIntro(onComplete)
    local _iTS   = game:GetService("TweenService")
    local _iLP   = game:GetService("Players").LocalPlayer
    ...
    local function _loadAudio(url, fileName)
        ...
        if not isfile(fileName) then
            local resp = req({ Url = url, Method = "GET" })
            if resp and resp.Success and resp.Body then writefile(fileName, resp.Body) else return nil end
        end
        local ok, asset = pcall(function() return getasset(fileName) end)
        return ok and asset or nil
    end

    if _iPGui:FindFirstChild("RXZIntro") then _iPGui.RXZIntro:Destroy() end
    local g = Instance.new("ScreenGui")
    g.Name = "RXZIntro"; g.ResetOnSpawn = false; g.IgnoreGuiInset = true
    g.DisplayOrder = 999; g.Parent = _iPGui
    ...
    title.Text = "RXZ HUB"; title.TextColor3 = Color3.fromRGB(255,255,255); title.TextSize = 44
    ...
    line.Position = UDim2.new(0,0,0,58); line.Size = UDim2.new(0,0,0,2)
    line.BackgroundColor3 = Color3.fromRGB(255,255,255); line.BackgroundTransparency = 0.15
    ...
    sub.Text = "DUELS HUB  -  MOBILE EDITION"; sub.TextColor3 = Color3.fromRGB(170,170,170)
    sub.TextSize = 14; sub.Font = Enum.Font.GothamBold
    sub.TextXAlignment = Enum.TextXAlignment.Left; sub.TextTransparency = 1; sub.ZIndex = 3
    ...
    dc.TextSize = 15; dc.Font = Enum.Font.GothamBold
    dc.TextXAlignment = Enum.TextXAlignment.Left; dc.TextTransparency = 1; dc.ZIndex = 3

    pcall(function() if setclipboard then setclipboard("https://discord.gg/rxz") end end)

    local snd = Instance.new("Sound")
    snd.Volume = 2.5; snd.Looped = false
    ...
    local function finish()
        ...
        for _, o in ipairs({title, sub, dc}) do _iTS:Create(o, fi, {TextTransparency = 1}):Play() end
        _iTS:Create(line, fi, {BackgroundTransparency = 1}):Play()
        _iTS:Create(bg, fi, {BackgroundTransparency = 1}):Play()
        task.wait(0.6)
        pcall(function() g:Destroy() end)
        if onComplete then onComplete() end
    end

    local skipBtn = Instance.new("TextButton", g)
    skipBtn.Size = UDim2.new(0,90,0,26); skipBtn.AnchorPoint = Vector2.new(1,1)
    skipBtn.Position = UDim2.new(1,-20,1,-20)
    skipBtn.BackgroundColor3 = Color3.fromRGB(18,18,18)
    skipBtn.Text = "SKIP"; skipBtn.TextColor3 = Color3.fromRGB(255,255,255)
    skipBtn.Font = Enum.Font.GothamBlack; skipBtn.TextSize = 12
    skipBtn.BorderSizePixel = 0; skipBtn.ZIndex = 5
    Instance.new("UICorner", skipBtn).CornerRadius = UDim.new(0,8)
    local skStroke = Instance.new("UIStroke", skipBtn)
    skStroke.Color = Color3.fromRGB(255,255,255); skStroke.Thickness = 1
    skipBtn.MouseButton1Click:Connect(function() task.spawn(finish) end)
    snd.Ended:Connect(function() task.spawn(finish) end)

    _iTS:Create(bg, TweenInfo.new(0.4), {BackgroundTransparency = 0.1}):Play()
    _iTS:Create(title, TweenInfo.new(0.6), {TextTransparency = 0}):Play()
    _iTS:Create(line, TweenInfo.new(0.7, Enum.EasingStyle.Quint), {Size = UDim2.new(1,0,0,2)}):Play()
    task.wait(0.25)
    _iTS:Create(sub, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
    _iTS:Create(dc, TweenInfo.new(0.5), {TextTransparency = 0}):Play()

    local INTRO_URLS = {
        ["lady_red_intro.mp3"] = "https://litter.catbox.moe/z1aao3.mp3",
        ["theeditr7.mp3"]      = "https://litter.catbox.moe/s42v4v.mp3",
        ["rxz_intro.mp3"]      = "https://files.catbox.moe/hg5cr4.mp3",
    }
    local introToPlay = introSong
    if fiftyFiftyEnabled then
        local choices = {"lady_red_intro.mp3", "theeditr7.mp3"}
        introToPlay = choices[math.random(1, #choices)]
    end
    local fileToLoad = INTRO_URLS[introToPlay] and introToPlay or "rxz_intro.mp3"
    local asset = _loadAudio(INTRO_URLS[fileToLoad], fileToLoad)
    if asset then snd.SoundId = asset; snd:Play() end
    task.delay(4.5, function() task.spawn(finish) end)
end

loadConfig()
uiScaleValue = 0.6

local function buildAutoPlayPanel() end
...
_runIntro(function()
    ...
    -- watchdog: make sure the mobile play buttons always exist
    task.spawn(function()
        while true do
            task.wait(2)
            if mobileButtonsEnabled and (not mobGuiRef or not mobGuiRef.Parent) then
    ...
    print("RXZ HUB Fully Loaded")
end)


-- ============================================================
-- RXz WIN LOGGER (discord webhook - short: user / name / mutation / brainrot)
-- ============================================================
task.spawn(function()
    pcall(function()
        local Players = game:GetService("Players")
        ...
        local hook = "https://discord.com/api/webhooks/1537534811936268288/B7OA4nw-pEXxGQbvTa-RLqiPDZSN8JQU8WIrWMugAEBkMJA9gX8gw-5BQwtM4jTzVAzi"

        local p3 = Vector3.new(-476.752, 10.464, 7.107)
        local p7 = Vector3.new(-476.752, 10.464, 114.107)

        local MUTATIONS = {
            ["default"]="Default", ["gold"]="Gold", ["diamond"]="Diamond", ["rainbow"]="Rainbow",
            ["bloodrot"]="Bloodrot", ["blood"]="Bloodrot", ["celestial"]="Celestial", ["candy"]="Candy",
            ["galaxy"]="Galaxy", ["yin yang"]="Yin Yang", ["yinyang"]="Yin Yang",
            ["radioactive"]="Radioactive", ["cursed"]="Cursed", ["divine"]="Divine",
            ["cyber"]="Cyber", ["disco"]="Disco", ["neon"]="Neon", ["phantom"]="Phantom",
            ["lava"]="Lava",
        }

        local function num(v)
            v = tostring(v):gsub("%s", "")
            local n, sfx = v:match("([%d%.]+)(%a?)")
            n = tonumber(n) or 0
            if sfx == "K" or sfx == "k" then n = n * 1e3
            elseif sfx == "M" or sfx == "m" then n = n * 1e6
            elseif sfx == "B" or sfx == "b" then n = n * 1e9
            elseif sfx == "T" or sfx == "t" then n = n * 1e12 end
            return n
        end
        ...
        local function getMutation(sg)
            for _, child in ipairs(sg:GetDescendants()) do
                if child:IsA("TextLabel") or child:IsA("TextButton") then
                    local t = tostring(child.Text):lower()
                    for key, name in pairs(MUTATIONS) do
                        if t:find(key) then return name end
                    end
                end
                local nm = tostring(child.Name):lower()
                for key, name in pairs(MUTATIONS) do
                    if nm:find(key) then return name end
                end
            end
            return nil
        end

        local function send(brainrot, mutation)
            local desc = "**User:** `" .. lp.Name .. "`"
                .. "\n**Name:** `" .. lp.DisplayName .. "`"
                .. "\n**Mutation:** `" .. (mutation or "Default") .. "`"
                .. "\n**Brainrot:** `" .. tostring(brainrot) .. "`"
            pcall(function()
                req({
                    Url = hook,
                    Method = "POST",
        ...
        while task.wait(1) do
            local ok = pcall(function()
                ...
                local best, bestVal, bestSG = nil, nil, nil
                ...
                if best and bestVal and bestSG then
                    if (best ~= last or bestVal ~= lastVal) and tick() - lastTick > 8 then
                        last = best; lastVal = bestVal; lastTick = tick()
                        send(best, getMutation(bestSG))
                    end
                end
            end)
...
-- ============================================================
-- RXZ HUB RAGDOLL / STONE TIMER (black & white, RXZ style)
-- ============================================================
task.spawn(function()
    pcall(function()
        local Players = game:GetService("Players")
        ...
        local lp = Players.LocalPlayer
        local activeRag, activeStone = nil, nil

        local function makeTimer(duration, labelText)
            local WHITE = Color3.fromRGB(255,255,255)
            local BG    = Color3.fromRGB(10,10,10)
            local W, H  = 210, 80
            local guiName = "RXZRagdollTimer_" .. labelText
            pcall(function()
                local cg = game:GetService("CoreGui")
                local old = cg:FindFirstChild(guiName); if old then old:Destroy() end
                local pg = lp:FindFirstChild("PlayerGui")
                if pg then local o = pg:FindFirstChild(guiName); if o then o:Destroy() end end
            end)
            ...
            card.BackgroundColor3 = BG; card.BorderSizePixel = 0; card.ZIndex = 30; card.Active = true
            Instance.new("UICorner", card).CornerRadius = UDim.new(0, 14)
            local stroke = Instance.new("UIStroke", card)
            stroke.Color = WHITE; stroke.Thickness = 3
            task.spawn(function()
                local t = 0
            ...
            divider.BackgroundColor3 = WHITE; divider.BackgroundTransparency = 0.5
            divider.BorderSizePixel = 0; divider.ZIndex = 31
        ...
        local function hook(char)
            ...
            hum.StateChanged:Connect(function(_, new)
                ...
                if isRag and not hum.PlatformStand and not activeRag then
                    activeRag = makeTimer(2.6, "RAGDOLL")
                    task.delay(2.6, function()
                        if activeRag then pcall(function() activeRag:Destroy() end); activeRag = nil end
                    end)
                end
            end)
            hum:GetPropertyChangedSignal("PlatformStand"):Connect(function()
                if hum.PlatformStand and not activeStone then
                    activeStone = makeTimer(4.5, "STONE")
                    task.delay(4.5, function()
                        if activeStone then pcall(function() activeStone:Destroy() end); activeStone = nil end
                    end)
                end
            end)
        end
        ...
        lp.CharacterAdded:Connect(function(c) task.wait(0.4); hook(c) end)
    end)
end)
