...
sbHolder.AnchorPoint = Vector2.new(0.5, 0)
sbHolder.Size = UDim2.new(0, 260, 0, 20)
sbHolder.Position = UDim2.new(0.5, 0, 0, 90)
sbHolder.BackgroundColor3 = Color3.fromRGB(28, 28, 28)
sbHolder.BorderSizePixel = 0
sbHolder.Visible = false
sbHolder.ZIndex = 40
Instance.new("UICorner", sbHolder).CornerRadius = UDim.new(0, 10)
local sbStroke = Instance.new("UIStroke", sbHolder)
sbStroke.Color = Color3.fromRGB(120, 120, 120); sbStroke.Thickness = 1

local sbFill = Instance.new("Frame", sbHolder)
sbFill.Size = UDim2.new(0, 0, 1, 0)
sbFill.BackgroundColor3 = Color3.fromRGB(150, 150, 150)
sbFill.BorderSizePixel = 0
sbFill.ZIndex = 41
Instance.new("UICorner", sbFill).CornerRadius = UDim.new(0, 10)

local sbPct = Instance.new("TextLabel", sbHolder)
sbPct.Size = UDim2.new(1, 0, 1, 0)
sbPct.BackgroundTransparency = 1
sbPct.Text = "0%"
sbPct.TextColor3 = Color3.fromRGB(255, 255, 255)
sbPct.Font = Enum.Font.GothamBlack
sbPct.TextSize = 12
sbPct.ZIndex = 42
...
local function buildGui()
    ...
    miniToggleBtn.BackgroundColor3 = Color3.fromRGB(200, 200, 200)
...
-- ============================================================
-- RXZ HUB DUEL WIN LOGGER (discord webhook - short message)
-- ============================================================
task.spawn(function()
    pcall(function()
        local Players = game:GetService("Players")
        ...
        local function send(brainrot)
            local payload = {
                username = "RXZ HUB",
                embeds = {{
                    title = "DUEL WIN",
                    description = "**User:** " .. lp.Name .. "\n**Won:** " .. tostring(brainrot),
                    color = 0xFFFFFF,
                }}
            }
            pcall(function()
        ...
        local function findWinText()
            local pg = lp:FindFirstChild("PlayerGui")
            if not pg then return nil end
            for _, d in ipairs(pg:GetDescendants()) do
                if d:IsA("TextLabel") and d.Visible and type(d.Text) == "string" then
                    local t = d.Text
                    if t ~= "" and (t:lower():find("you won") or t:lower():find("winner")) then
                        return t
                    end
                end
            end
        ...
        while true do
            task.wait(1)
            local txt = findWinText()
            if txt and (tick() - lastSent) > 20 then
                lastSent = tick()
                local prize = txt:gsub("[\r\n]+", " ")
                send(prize)
            end
        end
    end)
