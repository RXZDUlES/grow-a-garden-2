...
-- ============================================================
-- PALETĂ RXZ – NEGRU / ALB
-- ============================================================
local EDGE_BLACK      = Color3.fromRGB(0, 0, 0)
local MAIN_TEXT       = Color3.fromRGB(255, 255, 255)
local MUTED_TEXT      = Color3.fromRGB(255, 255, 255)
local STATUS_GREEN    = Color3.fromRGB(0, 255, 140)
local STATUS_RED      = Color3.fromRGB(255, 70, 100)
local GLASS_COLOR     = Color3.fromRGB(12, 8, 20)
...
pcall(function()
    for _, old in ipairs(LocalPlayer:WaitForChild("PlayerGui"):GetChildren()) do
        if old.Name == "BananaHubAntiBat" or old.Name == "EternalHubAntiBat" or old.Name == "EthernalHubAntiBat" or old.Name == "RedbullAntiBat" or old.Name == "RXZAntiBat" then
            old:Destroy()
        end
    end
end)
...
-- Generator de rânduri (borduri negre)
...
local function setVisuals(dot, pillStr, enabled)
    ...
    TweenService:Create(pillStr, TweenInfo.new(0.2), {
        Color = enabled and STATUS_GREEN or EDGE_BLACK
    }):Play()
end
...
local function updateUI()
    if AntiBatEnabled then
        ...
        toggleBtnStroke.Color = EDGE_BLACK
        toggleBtn.Text = "ACTIVATE ANTI BAT"
        statusTxt.Text = "STATUS: OFF"
        statusTxt.TextColor3 = STATUS_RED
    end
